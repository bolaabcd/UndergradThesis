...
\begin{enumerate}
\item Algorithm does not reflect the data.
    \begin{enumerate}
    \item Prefers the majority (Aggregation Bias)
    \item Systematic errors in the algorithm
    \item 
    \end{enumerate}
\item Data is biased, doesn't reflect reality
    \begin{enumerate}
    \item Society is biased, people didn't have a chance through life 
    \item Society is biased, people are good but didn't get the job
    \item Data collection doesn't reflect the reality: Measurement bias (for instance, COMPASS used friend/family arrests as a proxy for a risk score), Ommited Variable bias (this violates assumptions of some learning models, for instance linear regression models usually assumes error terms uncorrelated with the parameters considered in the regression), Representation/Sampling Bias (biased sampling lacking the diversity of the population), Simpson's Paradox (if we don't have data on a confounder, correlations might be spurious \cite{Causality}). 
    \item Population Bias (data was collected on another group)
    \item Temporal Bias (data collected on another time)
    \item Data that relies on people's opinion is prone to many biases (social bias people do what others are doing, self-selection bias people think that everyone agrees with them, and many many many others)
    \end{enumerate}
\item No exploration and data depends on the algorithm's output: Presentation Bias (the user only sees some cases), Ranking Bias (search engines presenting some results in a biased way), Popularity Bias (more popular are shown more)
\item Emergent Bias (things change, the algorithm not, and things might )
\end{enumerate}
